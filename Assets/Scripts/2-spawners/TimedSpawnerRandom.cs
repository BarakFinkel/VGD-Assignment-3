...
public class TimedSpawnerRandom: MonoBehaviour {
    ...
    [Tooltip("Minimum time between consecutive spawns, in seconds")] [SerializeField] protected float minTimeBetweenSpawns = 0.2f;
    [Tooltip("Maximum time between consecutive spawns, in seconds")] [SerializeField] protected float maxTimeBetweenSpawns = 1.0f;
    [Tooltip("Maximum distance in X between spawner and spawned objects, in meters")] [SerializeField] protected float maxXDistance = 1.5f;
    [SerializeField] protected Transform parentOfAllInstances;

    void Start() {
         SpawnRoutine();
    }

    virtual async protected void SpawnRoutine() {
        while (true) {
